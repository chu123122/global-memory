...
| Owner decision / publish-scope governance | `publish_scope_boundary` 未定；当前 tracked private paths = 175，unclassified tracked paths = 0 | `check_publish_scope.py --strict`: `private_tracked_paths=175`, `unclassified_tracked_paths=0`；`release_issue_ledger`: `oss-publish_scope` open | 项目 owner 决定 clean source repo、迁出 private 数据、fixture 化或保持私有审计 |
