...
2. 改用 `<task-id> 1 done` → `ERROR: design/ not under D:\global-memory\<task-id>`
...
2. **task 定位用 cwd 拼接**：脚本把第一个参数当 `cwd/<arg>` 去找 `design/`。任务在 `D:\ClaudeTasks\active\` 而非 cwd（如 `D:\global-memory`）下时，必须传**任务目录绝对路径**，不能只传 task-id。
